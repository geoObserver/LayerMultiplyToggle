...
- **Dropdown-Menü am Werkzeugknopf** (Pfeil neben dem Icon): *Show layer icons* (ein-/ausschaltbar, global gespeichert) blendet die Icons im Layerbaum sofort ein/aus, ohne die Funktion zu beeinflussen; *Reset* stellt alle ursprünglichen Mischmodi wieder her, entfernt alle Plugin-Icons aus dem Layerbaum, löscht den gespeicherten Status und schaltet den Toggle auf „aus".
...
5. Der **Pfeil neben dem Werkzeugknopf** öffnet ein Dropdown-Menü: *Show layer icons* schaltet die Anzeige der Layerbaum-Icons sofort ein/aus (global gespeichert, beeinflusst die Funktion nicht); *Reset* setzt alles zurück (Original-Mischmodi wiederhergestellt, alle Layerbaum-Icons entfernt, Status gelöscht, Toggle „aus").
...
- **Dropdown menu on the toolbar button** (arrow next to the icon): *Show layer icons* (toggle, stored globally) shows/hides the layer-tree icons instantly without affecting the function; *Reset* restores every layer's original blend mode, removes all plugin icons from the layer tree, clears the stored state and switches the toggle off.
...
5. The **arrow next to the toolbar button** opens a dropdown menu: *Show layer icons* toggles the display of the layer-tree icons instantly (stored globally, does not affect the function); *Reset* undoes everything (original blend modes restored, all layer-tree icons removed, stored state cleared, toggle off).
